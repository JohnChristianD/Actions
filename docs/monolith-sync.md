...
The finite-observation information boundary is stated with an explicit global quantifier: for any fixed canonical kernel and initial state, no observe : CanonicalFullLearnerState → Int8 has a global inverse : Int8 → CanonicalFullLearnerState satisfying inverse (observe s) ≡ s for every canonical state. The Nat-clock orbit supplies the finite-carrier contradiction, while the conclusion itself is global.

The stationary-limit seam remains explicit rather than being inferred from boundedness or a monotone-energy argument: `FiniteObservationStationaryLimitTheorem` requires the transition law, trajectory convergence, and preservation of the limit by the transition operator. The canonical Agda surface does not currently expose a standalone `MonotoneConvergenceToStationaryDistributionTheorem`, so the graph does not pre-graphedly require one.
...
The current Mercury graph-search stationary path is deliberately Lyapunov-free: `FiniteObservationStationaryLimitTheorem` makes the transition law, trajectory-convergence relation, and limit-preservation condition explicit. The theorem graph therefore does not invent or require a `CanonicalMonotoneEnergyStationarySubcompositionTheorem` that is absent from the canonical Agda surface.
