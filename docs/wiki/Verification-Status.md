...
The current raw grammar is deliberately small: NormPair replacement and the existing period-4 clock transformation, crossed with target invariance, full-step equivariance, and iterate equivariance. Iterate forms are quotient-pruned when the corresponding full-step form is already represented.
...
The first novel basis generated by the new search surface is:
...
- canonical full-step equivariance under the period-4 clock replacement.

These declarations are intentionally emitted into a generated Agda module rather than added automatically to the canonical theorem monolith. They become part of the canonical theorem source only after deliberate review.
