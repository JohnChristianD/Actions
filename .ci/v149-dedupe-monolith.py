...
# Remove the malformed audited-KKT documentation block only. The executable
# constructive KKT theorem remains later in the monolith.
...
# Script invariants. These are intentionally structural and idempotent.
if s.count('record SmoothAlgebra : Set₁ where') < 1:
    raise SystemExit('SmoothAlgebra record disappeared')
if s.count(marker_start) != 1:
    raise SystemExit(f'canonical SmoothAlgebra marker count={s.count(marker_start)}')
...
p.write_text(s)
print('monolith canonical algebra/parser/residual/q-cross/reciprocal-cross/multiplier/reciprocal-nonnegative/audited-KKT/terminal normalization applied deterministically')
