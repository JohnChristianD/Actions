...
('coeff (mul x y) ρ i = eval y ρ * coeff x ρ i + eval x ρ * coeff y ρ i', 'coeff (mul x y) ρ i = Ring._+_ Rg (Ring._*_ Rg (eval y ρ) (coeff x ρ i)) (Ring._*_ Rg (eval x ρ) (coeff y ρ i))'),
...
('      ; back = λ c → addCot (back px (c * vy)) (back py (c * vx))', '      ; back = λ c → addCot (back px (Ring._*_ Rg c vy)) (back py (Ring._*_ Rg c vx))'),
('    in record { value = neg (value px) ; back = λ c → negCot (back px c) }', '    in record { value = Ring.neg Rg (value px) ; back = λ c → negCot (back px c) }'),
('    in record { value = exp vx ; back = λ c → back px (c * dexp vx) }', '    in record { value = SmoothAlgebra.exp S vx ; back = λ c → back px (Ring._*_ Rg c (SmoothAlgebra.dexp S vx)) }'),
('    in record { value = log vx ; back = λ c → back px (c * dlog vx) }', '    in record { value = SmoothAlgebra.log S vx ; back = λ c → back px (Ring._*_ Rg c (SmoothAlgebra.dlog S vx)) }'),
('    in record { value = tanh vx ; back = λ c → back px (c * dtanh vx) }', '    in record { value = SmoothAlgebra.tanh S vx ; back = λ c → back px (Ring._*_ Rg c (SmoothAlgebra.dtanh S vx)) }'),
('    in record { value = sigmoid vx ; back = λ c → back px (c * dsigmoid vx) }', '    in record { value = SmoothAlgebra.sigmoid S vx ; back = λ c → back px (Ring._*_ Rg c (SmoothAlgebra.dsigmoid S vx)) }'),
('  vjpCoeff : ∀ e ρ c i → Pullback.back (pull e ρ) c i ≡ c * coeff e ρ i', '  vjpCoeff : ∀ (e : Expr) (ρ : Env) (c : CR) (i : Fin n) → Pullback.back (pull e ρ) c i ≡ Ring._*_ Rg c (coeff e ρ i)'),
]
