...
pop_best_acc([Candidate | Rest], Best0, Acc0, Best, Remaining) :-
    (
        if node_before(Candidate, Best0) then
...
expand_dependencies([Dependency | Dependencies], Node, Acc0, Children) :-
    Plan0 = plan(Node),
    (
        if list.member(Dependency, Plan0) then
...
goal_node(Node, Laws) :-
    Plan0 = plan(Node),
    list.length(Plan0) >= goal_depth,
    all_unique(Plan0),
    Plan0 = [TerminalId | _],
    law_for_id(seed(Node), Laws, SeedLaw),
...
search_emergent_compositions(Laws, MaxResults, Results, !IO) :-
    ...
    astar_collect(
        Laws, Seeds, 0, 1000, MaxResults, [], Reversed, !IO),
...
search_det(Laws, [First | Rest], Expansions, Plan) :-
    ...
    (
        if goal_node(Node, Laws) then
            ...
            search_det(Laws, Frontier1, Expansions + 1, Plan)
    ).
