...
RESIDUAL_NEW = '''residualSquareNonzero_v140 ha hr hx =
  λ x0 → OrderedRing.notLtFromLe (SmoothAlgebra.orderedRing S) ha
...
for path in Path('.').rglob('*.agda'):
    ...
    new, _ = RESIDUAL_RE.subn(RESIDUAL_NEW, new, count=1)
    if new != text:
        path.write_text(new)
        changed += 1
