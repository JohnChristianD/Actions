...
replacements = [
    (r'  accumulate i c \(state s\) = state \(λ j with finDecEq j i\n    \.\.\. \| yes _ = s j \+ c\n    \.\.\. \| no _ = s j\)', '''  accumulate i c (state s) = state (λ j → addAt j)
    where
    addAt : Fin n → R
    addAt j with finDecEq j i
    ... | yes _ = s j + c
    ... | no _ = s j'''),
    (r'insertCVT_v142 D a i f = record \{ cell = λ j with finDecEq i j\n.*?\.\.\.\s*\| no _ = CVTArchive_v142\.cell a j \}', '''insertCVT_v142 D a i f = record { cell = choose i }
  where
  ...
  ...     | no _ = CVTArchive_v142.cell a j }'''),
    (r'residualSquareNonzero_v140 ha hr hx =\n.*?(?=\n------------------------------------------------------------------------\n)', '''residualSquareNonzero_v140 ha hr hx = λ x0 →
  ...
  hzero = trans
    ...
    (r'orderedFieldCrossStrict_v142 a b d e hd he h =\n.*?(?=\n------------------------------------------------------------------------\n)', '''orderedFieldCrossStrict_v142 a b d e hd he h =
  ...
  rightNorm = trans (Ring.mulComm Rg c (b * SmoothAlgebra.recip _ e))
    ...
    (r'multiplierDeletionStrict_v142 n d y z hd he h =\n.*?(?=\n------------------------------------------------------------------------\n)', '''multiplierDeletionStrict_v142 n d y z hd he h =
...
for path in Path('.').rglob('*.agda'):
    ...
    for pattern, replacement in replacements:
        new, _ = replace_once(new, pattern, replacement)
    if new != text:
        path.write_text(new)
        changed += 1
