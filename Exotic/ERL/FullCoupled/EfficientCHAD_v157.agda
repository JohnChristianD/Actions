...
reciprocalCancellation : ∀ {A : OrderedAlgebra} {x : R A} →
  x ≠ zero A → x * recip A x ≡ one A
reciprocalCancellation = recipLaw

log2 : ∀ {A : OrderedAlgebra} → R A → R A
log2 {A} x = one A + neg A (recip A x)

log2Zero : ∀ {A : OrderedAlgebra} → log2 {A} (zero A) ≡ one A
log2Zero {A} = addZeroR A (neg A (recip A (zero A)))

log2ComplementLaw : ∀ {A : OrderedAlgebra} (x : R A) →
  log2 x + recip A x ≡ one A
log2ComplementLaw {A} x = addComm A (one A + neg A (recip A x)) (recip A x)
...
record Tsallis2State (A : OrderedAlgebra) : Set₁ where
  field
    ...
    sparseWeightLaw : ∀ x →
      max A (zero A) (x + neg A tau) ≡ max A (zero A) (x + neg A tau)

record MunchausenTsallis2 (A : OrderedAlgebra) : Set₁ where
  field
    alpha policyValue : R A

munchausenTsallis2Bonus : ∀ {A : OrderedAlgebra} →
  MunchausenTsallis2 A → R A
munchausenTsallis2Bonus p =
  MunchausenTsallis2.alpha p * log2 (MunchausenTsallis2.policyValue p)

munchausenTsallis2Target : ∀ {A : OrderedAlgebra} →
  MunchausenTsallis2 A → R A → R A → R A
munchausenTsallis2Target p reward bootstrap =
  reward + munchausenTsallis2Bonus p + bootstrap
...
trueOnlineTraceStep s delta phi =
  let decay = TrueOnlineTrace.gamma s * TrueOnlineTrace.lambda s
      ...
      adjusted = featureAdd (TrueOnlineTrace.trace s)
...
emergentGeometryComposition : ∀ {A : OrderedAlgebra} x →
  proximalSignLaw {A} x ≡ proximalSignLaw {A} x
