...
signVec : ∀ {A : OrderedAlgebra} {n : Nat} →
  SignActivationCertificate A → Vector A n → Vector A n
signVec s [] = []
signVec s (x ∷ xs) = sign s x ∷ signVec s xs

signMatrix : ∀ {A : OrderedAlgebra} {m n : Nat} →
  SignActivationCertificate A → Matrix A m n → Matrix A m n
signMatrix s [] = []
...
signDoubleVec : ∀ {A : OrderedAlgebra} {n : Nat}
  (s : SignActivationCertificate A) (x : Vector A n) →
...
signDoubleMatrix : ∀ {A : OrderedAlgebra} {m n : Nat}
