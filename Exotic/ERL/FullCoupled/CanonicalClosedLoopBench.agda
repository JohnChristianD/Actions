...
parity : Nat → Fin 2
parity zero = fromℕ< (m%n<n 0 2)
parity (suc zero) = fromℕ< (m%n<n 1 2)
parity (suc (suc n)) = parity n

policyAction2 : FullLearnerState → Fin 2
policyAction2 = canonicalBit learnerKernel

policyAction4 : FullLearnerState → Fin 4
policyAction4 s = fromℕ< (m%n<n
  ((cycle2 (clock s) * 2) + toℕ (canonicalBit learnerKernel s)) 4)
  where
    cycle2 : Nat → Nat
    cycle2 zero = zero
...
policyAction5 : FullLearnerState → Fin 5
policyAction5 s = fromℕ< (m%n<n
  ((cycle4 (clock s) * 2) + toℕ (canonicalBit learnerKernel s)) 5)

policyAction6 : FullLearnerState → Fin 6
policyAction6 s = fromℕ< (m%n<n
  ((cycle3 (clock s) * 2) + toℕ (canonicalBit learnerKernel s)) 6)
...
pongMiscSpec = closedLoopSpec
  ...
  8 8
  (λ s → fromℕ< (m%n<n
    (toℕ (canonicalBit learnerKernel s) + cycle3 (clock s)) 3))
  parity
  P.pongStep
...
closedLoopStep-progress : ∀ (s : FullLearnerState) (a : Fin 2) (r : Int8) →
