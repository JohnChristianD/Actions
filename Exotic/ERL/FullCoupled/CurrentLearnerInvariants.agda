...
module Exotic.ERL.FullCoupled.CurrentLearnerInvariants where
...
data Reach : State → State → Set where
  here : ∀ {s} → Reach s s
  there : ∀ {s t u r phi nextPhi} →
    step r phi nextPhi s ≡ t →
    Reach t u →
    Reach s u
...
networkReachInvariant (there h r) =
  trans (sym (networkPreserved _ _ _ _)) (networkReachInvariant r)
