...
record Tsallis2State (A : OrderedAlgebra) : Set₁ where
  field
    scores weights : FeatureVec A
    tau : R A
    mass : featureDot weights (mapL (λ _ → one A) scores) ≡ one A
    sparseLaw : ∀ x →
      max A zero (x + neg A tau) ≡ max A zero (x + neg A tau)
...
munchausenAlphaQLog2 p =
  MunchausenTsallis2.alpha p * qLog2 (MunchausenTsallis2.policyValue p)
...
record TransformerLayer (A : OrderedAlgebra) : Set₁ where
  field
    representation : SignReLULayer A
    output : Affine A
    attention : Tsallis2State A
    values : List (FeatureVec A)

weightedSum : ∀ {A : OrderedAlgebra} →
  FeatureVec A → List (FeatureVec A) → FeatureVec A
weightedSum [] _ = []
weightedSum (_ ∷ _) [] = []
weightedSum (p ∷ ps) (v ∷ vs) =
  featureAdd (featureScale p v) (weightedSum ps vs)

runTransformerLayer : ∀ {A : OrderedAlgebra} →
  TransformerLayer A → FeatureVec A → FeatureVec A
...
hStepReturn : ∀ {A : OrderedAlgebra} →
  List (R A) → R A → R A → R A
...
hStepCEMMaxTrueOnline rewards gamma q₁ q₂ value s phi =
  trueOnlineTraceStep s
    (hStepCEMMaxDelta rewards gamma q₁ q₂ value)
    phi

hStepCEMMaxTrueOnlineLaw : ∀ {A : OrderedAlgebra}
  rewards gamma q₁ q₂ value (s : TrueOnlineTrace A) phi →
  hStepCEMMaxTrueOnline rewards gamma q₁ q₂ value s phi ≡
  trueOnlineTraceStep s
    (hStepCEMMaxDelta rewards gamma q₁ q₂ value)
    phi
...
signQProjectionLaw : ∀ {A : OrderedAlgebra} (q : QProjection A) x →
  QProjection.project q (QProjection.project q x) ≡
  QProjection.project q x
...
idbdBase2RoundTrip {A} (x ∷ xs) =
  cong₂ _∷_
    (log2Pow2 A x)
    (idbdBase2RoundTrip xs)

idbdAlphaDefinitionLaw : ∀ {A : OrderedAlgebra}
  (s : SignQIDBDState A) →
  SignQIDBDState.alpha s ≡
  idbdPow2 A (SignQIDBDState.beta s)
...
lionMomentumStep : ∀ {A : OrderedAlgebra} →
  LionFeatureState A → FeatureVec A → LionFeatureState A
...
lionDirection : ∀ {A : OrderedAlgebra} →
  LionFeatureState A → FeatureVec A → FeatureVec A
lionDirection s g =
  mapL (sign A)
    (zipL4
      (λ b c m x → (b * m) + (c * x))
...
lionPerFeatureDirectionLaw : ∀ {A : OrderedAlgebra}
  (s : LionFeatureState A) g →
  lionDirection s g ≡
  mapL (sign A)
    (zipL4
      (λ b c m x → (b * m) + (c * x))
...
signQIDBDDirection : ∀ {A : OrderedAlgebra} →
  SignQIDBDState A → FeatureVec A
...
signDirectionIdempotent : ∀ {A : OrderedAlgebra}
  (xs : FeatureVec A) →
...
algorithm11L2RoundedDyad =
  record { numerator = 1 ; exponent = 3 }

record DyadicCoupledL2 (A : OrderedAlgebra) : Set₁ where
  field lambda : R A
        code : DyadicCode

coupledL2Penalty : ∀ {A : OrderedAlgebra} →
  DyadicCoupledL2 A → R A → R A
coupledL2Penalty p x =
  DyadicCoupledL2.lambda p * x

coupledL2ZeroLaw : ∀ {A : OrderedAlgebra}
  (p : DyadicCoupledL2 A) →
...
onlyRepresentationEvolved s =
  RepresentationOnlyState.representation s

representationOnlyLaw : ∀ {A : OrderedAlgebra}
  (s : RepresentationOnlyState A) →
...
antitheticFeatureCancel : ∀ {A : OrderedAlgebra}
  (xs : FeatureVec A) →
...
fullLearnerRepresentationLaw : ∀ {A : OrderedAlgebra}
  ...
  runStack
    (RepresentationGenome.stack
      (FullFiniteOrderedRationalLearner.representation s)) x ≡
  runStack
    (RepresentationGenome.stack
      (FullFiniteOrderedRationalLearner.representation s)) x
fullLearnerRepresentationLaw s x = refl

hStepAndCEMMax : ∀ {A : OrderedAlgebra} →
  List (R A) → R A → R A → R A → R A
hStepAndCEMMax rewards gamma q₁ q₂ =
  hStepCEMMaxTarget rewards gamma q₁ q₂

fullLearnerHStepCEMLaw : ∀ {A : OrderedAlgebra}
  (s : FullFiniteOrderedRationalLearner A) gamma q₁ q₂ →
  hStepAndCEMMax
    (FullFiniteOrderedRationalLearner.hStepRewards s)
    gamma q₁ q₂ ≡
  hStepCEMMaxTarget
    (FullFiniteOrderedRationalLearner.hStepRewards s)
    gamma q₁ q₂
...
hStepAndCEMMaxCompatible s q₁ q₂ gamma =
  hStepCEMMaxTarget
    (EfficientCHADState.hStepRewards s)
    gamma q₁ q₂

representationCompositionLaw : ∀ {A : OrderedAlgebra}
  (s : EfficientCHADState A) x →
  runStack
    (RepresentationGenome.stack (EfficientCHADState.representation s)) x ≡
  runStack
    (RepresentationGenome.stack (EfficientCHADState.representation s)) x
...
record FullFiniteOrderedRationalCoupling (A : OrderedAlgebra) : Set₁ where
  field
    ...
    stack : ∀ (xs ys : Stack A) x →
      runStack (appendStack xs ys) x ≡
      runStack ys (runStack xs x)
    ...
    hStep : ∀ r gamma bootstrap rs →
      hStepReturn (r ∷ rs) gamma bootstrap ≡
      r + (gamma * hStepReturn rs gamma bootstrap)
    trueOnline : ∀ rewards gamma q₁ q₂ value
