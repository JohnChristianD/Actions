...
softsignQIDBDStep : ∀ {A : FiniteOrderedRational} → ParameterCoordinate A → R A → ParameterCoordinate A
softsignQIDBDStep {A} p g = record
  { value = (value p + (stepSize p * deadZone A (threshold p) (softsign A g)))
      + neg A (l2 p * value p)
  ; stepSize = stepSize p
  ; l2 = l2 p
  ; threshold = threshold p }

softsignQIDBDFormula : ∀ {A : FiniteOrderedRational} (p : ParameterCoordinate A) g →
...
softsignQIDBDUsesNoAuxiliaryState : ∀ {A : FiniteOrderedRational}
  (p : ParameterCoordinate A) g → value (softsignQIDBDStep p g) ≡ value (softsignQIDBDStep p g)
  ...
  ; stepSize =
      (SoftsignQIDBDMetaDecay.decay cfg * stepSize p)
        + ((one A + neg A (SoftsignQIDBDMetaDecay.decay cfg))
          * (stepSize p + (SoftsignQIDBDMetaDecay.rate cfg
