...
-- Munchausen is a Bellman-target construction: the immediate reward is
-- augmented by a scaled log-policy term before bootstrapping.  The
-- negative-Munchausen variant below is exactly the same formal construction
-- with the scale's sign flipped.  This is deliberately distinguished from
-- negating the q-logarithm itself.
...
-- The target is kept in a finite symbolic Bellman carrier.  The syntax makes
-- every ingredient visible without importing real-number or transcendental
-- arithmetic into the --safe learner kernel.
...
-- Endogenous scale: the current binary sparsemax/LCB head supplies the
-- magnitude.  The negative sign is a separate algebraic operation, so the
-- endogenous control is not confused with the sign of q-log itself.
...
-- q-log remains an ingredient term; its negative-Munchausen status is supplied
-- by the signed scale.  This avoids the naming error "negative q-log" for a
-- sign-flipped Munchausen coefficient.
