...
(defn first-negative [mask a x mu]
  (some (fn [i]
          (when (< (- (nth a i) (* mu (sq (nth x i)))) 0)
            i))
        mask))
(defn runq [mask a x b steps]
  (let [n0 (num mask a x b)
        d0 (den mask x)
        mu (if (and (pos? n0) (pos? d0)) (/ n0 d0) 0)
        bad (first-negative mask a x mu)]
    (if (nil? bad)
      [mu mask steps]
      (let [r (vec (remove #(= % bad) mask))
            n2 (num r a x b)
            d2 (den r x)
...
(defn vectors [n]
  (if (zero? n)
    [[]]
    (for [p (vectors (dec n)) q [0 1 2]] (conj p q))))
(doseq [n (range 1 5) a (vectors n) x (vectors n) bi (range 5)]
  (let [[mu active steps] (runq (vec (range n)) a x bi 0)]
    (assert (<= steps n))
    (doseq [i active]
      (assert (<= 0 (- (nth a i) (* mu (sq (nth x i)))))))
    (doseq [i (remove (set active) (range n))]
      (assert (<= (- (nth a i) (* mu (sq (nth x i)))) 0)))))
